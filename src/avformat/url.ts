...
export const URL_PROTOCOL_FLAG_NESTED_SCHEME = 1;

/*< The protocol uses network */
export const URL_PROTOCOL_FLAG_NETWORK =       2;

//extern const AVClass ffurl_context_class; // @nodeTODO
...
export interface URLProtocol {
    name: string;
    url_open: (h: URLContext, url: string, flags: number) => number;
    /**
     * This callback is to be used by protocols which open further nested
     * protocols. options are then to be passed to ffurl_open_whitelist()
     * or ffurl_connect() for those nested protocols.
     */
    url_open2: (h: URLContext, url: string, flags: number, options: Ref<AVDictionary>) => number;
    url_accept: (s: URLContext, c: Out<URLContext>) => number;
    url_handshake: (c: URLContext) => number;
     ...
     */
    url_read: ( h: URLContext, buf: Buffer, size: number) => number;
    url_write: (h: URLContext, buf: Buffer, size: number) => number;
    url_seek: ( h: URLContext, pos: number, whence: number) => number;
    url_close: (h: URLContext) => number;
    url_read_pause: (h: URLContext, pause: number) => number;
    url_read_seek: (h: URLContext, stream_index: number,
                            timestamp: number, flags: number) => number;
    url_get_file_handle: (h: URLContext) => number;
    url_get_multi_file_handle: (h: URLContext, handles: Out<number[]>,
                                    numhandles: Out<number>) => number;
    url_get_short_seek: (h: URLContext) => number;
    url_shutdown: (h: URLContext, flags: number) => number;
    priv_data_class: AVClass;
    priv_data_size: number;
    flags: number;

    url_check: (h: URLContext, mask: number) => number;
    url_open_dir: (h: URLContext) => number;
    url_read_dir: (h: URLContext, next: Ref<AVIODirEntry>) => number;
    url_close_dir: (h: URLContext) => number;
    url_delete: (h: URLContext) => number;
    url_move: (h_src: URLContext, h_dst: URLContext) => number;
 ...
 */
export function ff_url_join(
    str: string, size: number, proto: string,
    authorization: string, hostname: string,
